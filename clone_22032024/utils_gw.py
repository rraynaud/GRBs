...
def loop_fitsfile(folder="./", probs=[0.9]):
    ...
    sizemaps = []
    # make a list in case of multiple calls

    for filen in results:
        skymap = QTable.read(folder + "/" + filen)
        sizemap = get_prob_size(skymap,prob_lim=probs)
        sizemaps.append(sizemap)
        print(filen + " " + str(sizemap))

        time = filen[:9]
        # the gps time is the first 10 characters of the fits file name
        times.append(time)

    return times, sizemaps
